...
const hikes = [
   ...
   // the sort function will sort the list within the place of the original array, so that the original is changed each time.

//    Here is an example of how a sort/filter func would work within js:
...
// filter doesn't change the original array like sort does, it just compiles using the previous array instead compared to sort.


// how would I use filter within the list of strings.
// strings are declared 
function searchList(list, query) {
    function searchCallback(string) {
        return string.toLowerCase().includes(query.toLowerCase());
        // this lower cases the string and the query as well
    }
...
// building upon the previous searchList func by passing hikeList instead

function searchList(list, q) {
// How is this working on this assignment, 
    function searchCallback(item) {
        return (
            item.name.toLowerCase().includes(q.toLowerCase()) || 
            item.description.toLowerCase().includes(q.toLowerCase()) ||
            item.tags.find((tag) => tag.toLowerCase().includes(q.toLowerCase()))
            // need to include || the pipeline so that extra objects can be included within the returning of this object)
        );
        // this lower cases the string and the query as well
    }
...
// What does 'al' do when we are sorting this list, and then displaying it?
